...
const PLATFORMS: { id: Platform; label: string; color: string; icon: string; manual: boolean }[] = [
  ...
  { id: "tiktok",   label: "TikTok",   color: "from-black to-gray-700",   icon: "TT", manual: false },
  ...
  return (
    <div className="p-6 rounded-2xl border border-white/10 bg-white/4 space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-white">Social Platforms</h2>
      </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  {data.subscribers !== undefined && (
                    <div>
                      ...
                      <div className="text-xs text-white/30">Subs</div>
                    </div>
                  )}
      ...
      {!hasAny && !isOwner && (
        <p className="text-sm text-white/30 text-center py-4">No social platforms linked yet.</p>
      )}
              ...
              return (
                <>
                        ...
                        {fetchError && (
                          <p className="text-xs text-amber-400">
                            {fetchError.includes("not configured")
                              ? "Auto-fetch requires RAPIDAPI_KEY. Enter stats manually below."
                              : fetchError}
                          </p>
                        )}
                      </div>
                  ...
                  <div className="flex gap-2 mt-5">
                    <Button variant="ghost" className="flex-1" onClick={closeEditor}>Cancel</Button>
                    <Button
                      variant="gradient"
